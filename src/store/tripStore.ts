  ...
  persist(
    (set, get) => ({
      ...
      addDistance: (distance: number) => {
        ...
        logger(
          `CONSOLE: ${distance >= 0 ? 'Added' : 'Adjusted'} ${Math.abs(distance).toFixed(1)}km`
        );
      },
      ...
      setTotalTraveled: (distance: number) => {
        ...
        logger(
          `CONSOLE: Set total traveled distance to ${distance.toFixed(1)}km`
        );
      },
      ...
      importTripData: (data: any) => {
        try {
          ...
          if (
            ...
            typeof data.autoStartLocation === 'object'
          ) {
