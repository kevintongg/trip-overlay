 ...
 */
async function fetchWeatherDirect(lat: number, lon: number, units: string = 'metric'): Promise<WeatherResponse> {
...
function generateMockWeather(lat: number, lon: number): WeatherResponse {
  ...
  if (lon >= -10 && lon <= 40 && lat >= 35 && lat <= 70) {
    ...
    if (lon >= 5 && lon <= 25) {
      ...
      const viennaTime = new Date(utc.toLocaleString('en-US', { timeZone: 'Europe/Vienna' }));
      ...
      const now = new Date();
      const utc = new Date(now.getTime() + now.getTimezoneOffset() * 60000);
      const londonTime = new Date(utc.toLocaleString('en-US', { timeZone: 'Europe/London' }));
    ...
    const now = new Date();
    const utc = new Date(now.getTime() + now.getTimezoneOffset() * 60000);
    const nyTime = new Date(utc.toLocaleString('en-US', { timeZone: 'America/New_York' }));
 ...
 */
export async function fetchWeatherData(lat: number, lon: number, units: string = 'metric'): Promise<WeatherResponse> {
  console.log(`🌤️ Fetching weather for ${lat}, ${lon} (${units})`);
  
  // Strategy 1: Try Cloudflare function first
  try {
    const response = await fetch(`/functions/weather?lat=${lat}&lon=${lon}&units=${units}`);
    if (response.ok) {
      const responseText = await response.text();
      
      // Check if response is JavaScript source code instead of JSON
      if (responseText.trim().startsWith('//') || responseText.includes('function') || responseText.includes('export')) {
        console.warn('⚠️ Weather: Cloudflare function returned source code (not deployed properly)');
      } else {
        try {
          ...
          console.warn('⚠️ Weather: Cloudflare function returned invalid JSON:', responseText.substring(0, 100));
        }
      }
    } else {
      const errorText = await response.text();
      console.warn('⚠️ Weather: Cloudflare function failed:', response.status, errorText);
    }
  } catch (error) {
