        ...
        switch (key) {
          ...
          case 'resets':
            ...
            resetTypes.forEach(resetType => {
              if (resetType === 'all') {
                ...
                logger.warn('Unknown reset type in multiple resets:', resetType)
              }
            })
            processedParams++
            break
          ...
          case 'units':
            if (value === 'miles' || value === 'imperial') {
              ...
              logger.warn('Invalid units parameter:', value, '(must be miles, km, imperial, or metric)')
            }
            processedParams++
            break
          ...
          case 'totalDistance':
            ...
            if (!isNaN(totalDist) && totalDist >= 0 && totalDist <= 50000) {
              ...
              logger.warn('Invalid totalDistance parameter:', value, '(must be 0-50000)')
            }
            processedParams++
            break
          ...
          case 'addDistance':
            ...
            if (!isNaN(addDist) && addDist >= -10000 && addDist <= 10000) {
              logger(`URL parameter: ${addDist >= 0 ? 'Added' : 'Subtracted'} ${Math.abs(addDist)}km`)
              addDistance(addDist)
            } else {
              logger.warn('Invalid addDistance parameter:', value, '(must be -10000 to 10000)')
            }
            processedParams++
            break
          ...
          case 'setDistance':
            ...
            if (!isNaN(setDist) && setDist >= 0 && setDist <= 50000) {
              ...
              logger.warn('Invalid setDistance parameter:', value, '(must be 0-50000)')
            }
            processedParams++
            break
          ...
          case 'setTodayDistance':
            ...
            if (!isNaN(todayDist) && todayDist >= 0 && todayDist <= 1000) {
              ...
              logger.warn('Invalid setTodayDistance parameter:', value, '(must be 0-1000)')
            }
            processedParams++
            break
          ...
          case 'setTotalTraveled':
            ...
            if (!isNaN(totalTrav) && totalTrav >= 0 && totalTrav <= 50000) {
              logger(`URL parameter: Set total traveled distance to ${totalTrav}km`)
              setTotalTraveled(totalTrav)
            } else {
              logger.warn('Invalid setTotalTraveled parameter:', value, '(must be 0-50000)')
            }
            processedParams++
            break
